...
import pandas as pd
import numpy as np
data06 = pd.read_csv('Dataset/Dataset_06.csv')
...
q1 = data06.copy()
q1_out = q1['price'].groupby(q1['waterfront']).mean()
res = q1_out[0] - q1_out[1]
res = abs(res)
res
#1167272]

q1_0 = q1[q1.waterfront==0]['price'].mean()
q1_1 = q1[q1.waterfront==1]['price'].mean()
...
q2 = data06.copy()
q2_res = q2[['price', 'bedrooms', 'bathrooms', 'sqft_living', 'sqft_lot', 'floors', 'yr_built']].corr()

q2_res['price']

# sqft_living, yt_built
...
q3 = data06.copy()
x = q3.drop(columns=['id', 'date', 'zipcode', 'price'])
y = q3.price


data_x ='+'.join(x)
res = ols('price ~'+data_x,q3)
res = res.fit()
res.summary()

dir(res)
res.pvalues.drop('Intercept')[res.pvalues <0.05].count()
res.params[res.params<0].count() ## p_value가 유의미한것만 count 해야한다.
...
data07 = pd.read_csv('Dataset/Dataset_07.csv')
...
q1 = q1[var_list].corr().drop('Chance_of_Admit')['Chance_of_Admit']
abs(q1).max()
...
q2 = data07.copy()
q2['GRE_gr'] = np.where(q2.GRE>mu,1,0) #큰쪽이 1 작은쪽이 0
...
q3 = data07.copy()
q3['group'] = np.where(q3.Chance_of_Admit>0.5,1,0) #합격1 불합격 0


from sklearn.linear_model import LogisticRegression

x = q3.drop(columns=['group', 'Serial_No', 'Chance_of_Admit'])
y = q3.group
...
#q3_out = pd.DataFrame(data=[x.columns, logit.coef_.reshape(-1,)], columns= ['name', 'coef'])
q3_out = pd.DataFrame()
q3_out['name'] = x.columns
q3_out['coef'] =  logit.coef_.reshape(-1,)
#1.95, cgpa
...
data10.info()
data10.dropna(inplace=True, axis=1) #axis=1인경우 불필요한 열 제거가능
#%%
...
q1 = data10.copy()
q1_res = q1[(q1.previous_owners==1) & (q1.engine_power==51)].sort_values(by='price')
q1_res.head(1)['model']
# 960    pop

q1_tab = pd.pivot_table(q1_res, index='model',
                        values=['km','age_in_days'])
q1_tab['km_days'] = q1_tab.km / q1_tab.age_in_days
q1_tab['km_days'].min()/ q1_tab['km_days'].max()
#0.97
...
min_group = q1_tab['km_days'].idxmin()
max_group = q1_tab['km_days'].idxmax()
q2 = data10.copy()
...
##2개 그룹필터
max_km = q2[q2.model==min_group]['km_per_day']
min_km = q2[q2.model==max_group]['km_per_day']
...
x = q3[['engine_power', 'age_in_days', 'km']]
...
lr.predict(pd.DataFrame(new_data))
## 10367
